...
function parseCssRules(css) {
  ...
  while ((match = rulePattern.exec(css)) !== null) {
    ...
    if (selector) {
      rules.set(selector, parseDeclarations(match[2]));
    }
  }
...
function collectMarkupClassNames(html) {
...
function selectorPrimaryClassName(selector) {
  return selector.match(/\.([a-zA-Z0-9_-]+)/)?.[1] || null;
}
...
function verifyRoleRules(css, lock, sourceLabel, { renderedHtml = null } = {}) {
  ...
  const markupClassNames = renderedHtml ? collectMarkupClassNames(renderedHtml) : null;

  lock.roles.forEach((role) => {
    const selector = normalizeSelector(role.selector);
    const declarations = rules.get(selector);
    if (!declarations) {
      const primaryClassName = selectorPrimaryClassName(role.selector);
      const roleIsUsed = !markupClassNames
        || !primaryClassName
        || markupClassNames.has(primaryClassName);
      if (!roleIsUsed) return;
    ...
    Object.entries(role.declarations).forEach(([property, expectedValue]) => {
      ...
      if (actualValue !== normalizeDeclarationValue(expectedValue)) {
        throw new Error(
          `${sourceLabel} mobile typography drift for ${role.id} ${property}: expected "${expectedValue}", found "${actualValue || 'missing'}"`,
        );
      }
    });
  });
...
function verifySectionStyleTokens(sectionStyles, lock, sourceLabel) {
  Object.entries(lock.sectionStyleTokens || {}).forEach(([pathExpression, expectedValue]) => {
    ...
    if (actualValue !== expectedValue) {
      throw new Error(
        `${sourceLabel} mobile typography token drift for ${pathExpression}: expected "${expectedValue}", found "${actualValue ?? 'missing'}"`,
      );
    }
  });
}
