...
function verifyBaseRoleRules(renderedHtml, lock, sourceLabel) {
  ...
  (lock.baseRoles || []).forEach((role) => {
    ...
    if (elements.length === 0) {
      throw new Error(
        `${sourceLabel} is missing locked base typography selector for ${role.id}: ${role.selector}`,
      );
    }
    ...
    elements.forEach((element, index) => {
      ...
      Object.entries(role.declarations).forEach(([property, expectedValue]) => {
        ...
        if (actualValue !== normalizeDeclarationValue(expectedValue)) {
          throw new Error(
            `${sourceLabel} base typography drift for ${role.id}[${index}] ${property}: expected "${expectedValue}", found "${actualValue || 'missing'}"`,
          );
        }
      });
    });
...
export function verifyMobileTypographyLock({
  ...
  verifyBaseRoleRules(renderedHtml, entry, `${outputName} rendered HTML`);
