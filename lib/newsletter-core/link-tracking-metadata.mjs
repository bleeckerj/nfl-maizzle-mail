...
function collectIntroStatementHtmlLinkRecords(html, context) {
  if (typeof html !== 'string' || !/<a\b/i.test(html)) return;

  const { path, section, sectionIndex, sourcePath, lineForUrl, records } = context;
  ...
  anchors.forEach((anchor, index) => {
    ...
    records.push({
      ...
      category,
      path: [...path, `a[${index}]`],
      pathLabel: `${formatPath(path)}.a[${index}]`,
      sourcePath,
      line: lineForUrl(url),
      explicitLabel: Boolean(anchorText),
      explicitCategory: false,
      defaultedLabel: !anchorText,
...
function traverse(value, context) {
  ...
  for (const [key, child] of Object.entries(value)) {
    ...
    if (isIntroStatementHtmlField(key, section)) {
      collectIntroStatementHtmlLinkRecords(child, {
        ...context,
        path: childPath,
        section,
        sectionIndex,
