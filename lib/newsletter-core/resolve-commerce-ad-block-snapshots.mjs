...
const DEFAULT_UPLOAD_FOLDER = 'ads-snapshots';
const DEFAULT_PHOTARIUM_NAMESPACE = 'cf-speculative-ads';
...
function collectCommerceAdBlockTargets(newsletterData, adIndex) {
  ...
  newsletterData.sections.forEach((section, sectionIndex) => {
    ...
    section.items.forEach((item, itemIndex) => {
      ...
      const shouldRenderSnapshot = renderMode === 'snapshot';
...
function resolveTargetPreset(item, ad) {
  const commerceAspectRatio = normalizeSnapshotPreset(ad?.commerce?.lockup?.aspectRatio);
  if (ad?.commerce?.presentation === 'overlay-lockup' && commerceAspectRatio) {
    return commerceAspectRatio;
  }
...
function renderedPresetMatches(renderedPresetId, requestedPreset) {
  return (
    renderedPresetId === requestedPreset ||
    renderedPresetId === `social-${requestedPreset}` ||
    renderedPresetId === `custom-${requestedPreset}`
  );
}
...
export async function resolveCommerceAdBlockSnapshots(
  ...
  try {
    batch = await withWorkingDirectory(editorialRoot, async () =>
      editorialAdsSnapshotsRenderBatch({
        ...
        photariumNamespace: process.env.NFL_AD_SNAPSHOTS_PHOTARIUM_NAMESPACE || DEFAULT_PHOTARIUM_NAMESPACE,
        photariumFolder: process.env.NFL_AD_SNAPSHOTS_PHOTARIUM_FOLDER || DEFAULT_UPLOAD_FOLDER,
        lockupRenderer: 'preview-lockup',
      }),
    );
  } catch (error) {
    logger.log(
  ...
  for (const target of targets) {
    ...
    const rendered =
      snapshotResult?.rendered?.find((entry) => renderedPresetMatches(entry.presetId, target.preset)) ??
      snapshotResult?.skipped?.find((entry) => renderedPresetMatches(entry.presetId, target.preset))?.existing;
