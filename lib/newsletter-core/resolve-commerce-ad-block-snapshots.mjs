...
function selectEmailSnapshotArtifact(rendered) {
  return (
    rendered.carouselAnimatedWebpArtifact ||
    rendered.animatedWebpArtifact ||
    (isWebpArtifact(rendered.artifact) ? rendered.artifact : null) ||
    rendered.stillArtifact ||
    rendered.artifact
  );
}
...
export async function resolveCommerceAdBlockSnapshots(
  ...
  for (const target of targets) {
    ...
    const selectedArtifact = selectEmailSnapshotArtifact(rendered);
