...
function preserveCaptionParagraphs(caption) {
  const normalized = caption.trim();
  if (/<(?:p|div)\b/i.test(normalized)) {
    return normalized
      .replace(/<div\b/gi, '<p')
      .replace(/<\/div>/gi, '</p>');
  }
...
export function hydrateShortTakeSections(newsletterData, repoRoot, { logger = console } = {}) {
  ...
  newsletterData.sections.forEach((section, sectionIndex) => {
    ...
    const hydratedItem = {
      ...
      caption: preserveCaptionParagraphs(record.url ? captionWithoutNestedAnchors(record.caption) : record.caption),
