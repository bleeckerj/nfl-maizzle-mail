...
const AD_BLOCK_TRACKING_CATEGORY = 'ad-block';
...
export function hydrateAdBlockSections(newsletterData, repoRoot, { logger = console } = {}) {
  ...
  newsletterData.sections.forEach((section, sectionIndex) => {
    ...
    // Opt-in (per newsletter item): render this commerce ad as its pre-composited
    // lockup image (price + icon already baked in) instead of the plain product
    // image. We only swap the image — label and sponsor stay as live HTML text, so
    // the template renders the normal card with the lockup image in place of media.
    const lockupOptIn = sourceItem?.commerceLockup === true || sourceItem?.commerceLockup === 'true';
    const isOverlayLockup = ad?.commerce?.presentation === 'overlay-lockup';
    const lockupSnapshotSrc = typeof ad?.commerce?.lockup?.snapshotSrc === 'string'
      ? ad.commerce.lockup.snapshotSrc.trim()
      : '';
    const lockupSnapshotAlt = typeof ad?.commerce?.lockup?.snapshotAltText === 'string'
      ? ad.commerce.lockup.snapshotAltText.trim()
      : '';

    if (lockupOptIn && lockupSnapshotSrc) {
      hydratedItem.image = lockupSnapshotSrc;
      if (lockupSnapshotAlt) {
        hydratedItem.imageAlt = lockupSnapshotAlt;
      }
      logger.log(`🛍️  Ad-block "${adId}": showing its composited price+icon lockup image (commerceLockup: true).`);
    } else if (lockupOptIn && !lockupSnapshotSrc) {
      logger.log(`⚠️  Ad-block "${adId}": you set "commerceLockup: true" in the newsletter, but this ad has no baked lockup image yet. Bake one: run \`npm run ads:preview -- ${adId} --save-snapshot\` (or the "editorial_ads_preview_snapshot" MCP tool with upload: true), then paste the returned URL into "commerce.lockup.snapshotSrc" for this ad in nfl-editorial/src/content/ads.json. Showing the plain card for now. Guide: nfl-maizzle-mail/docs/COMMERCE-AD-LOCKUPS.md`);
    } else if (isOverlayLockup) {
      if (lockupSnapshotSrc) {
        logger.log(`💡 Ad-block "${adId}" can show its price+icon lockup. Add "commerceLockup: true" beside "adId" in this newsletter's ad-block item to switch it on (showing the plain card otherwise). Guide: nfl-maizzle-mail/docs/COMMERCE-AD-LOCKUPS.md`);
      } else {
        logger.log(`💡 Ad-block "${adId}" is a commerce ad (has a price + icon) but is showing as a plain image. To show the price+icon version: (1) bake the image — \`npm run ads:preview -- ${adId} --save-snapshot\` or the "editorial_ads_preview_snapshot" MCP tool with upload: true; (2) paste the returned URL into "commerce.lockup.snapshotSrc" for this ad in nfl-editorial/src/content/ads.json; (3) add "commerceLockup: true" beside "adId" in this newsletter's ad-block item. Guide: nfl-maizzle-mail/docs/COMMERCE-AD-LOCKUPS.md`);
      }
    }
    ...
    if (resolvedLinkUrl) {
      hydratedItem.link = buildTrackedLinkValue(
        ...
        {
          label: adId,
          category: AD_BLOCK_TRACKING_CATEGORY,
        },
      );
    }
    ...
    if (resolvedReadMoreLink) {
      hydratedItem.readMoreLink = resolvedReadMoreLink === resolvedLinkUrl && hydratedItem.link
        ...
        : buildTrackedLinkValue(
            ...
            [markdownReadMoreLink, markdownLink],
            {
              label: adId,
              category: AD_BLOCK_TRACKING_CATEGORY,
            },
