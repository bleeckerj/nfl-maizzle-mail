...
export function normalizeNewsletterTextBreaks(newsletterData) {
  ...
  const normalizeField = (owner, field) => {
    if (!owner || typeof owner !== 'object' || typeof owner[field] !== 'string') return;

    const normalized = normalizeAuthorHtmlBreaks(owner[field]);
    if (normalized !== owner[field]) {
      owner[field] = normalized;
      changes += 1;
    }
  };

  normalizeField(newsletterData.hero, 'body');

  if (!Array.isArray(newsletterData.sections)) return changes;
  ...
  newsletterData.sections.forEach((section) => {
    ...
    if (Array.isArray(section.items)) {
      section.items.forEach((item) => {
        normalizeField(item, 'body');
        normalizeField(item, 'description');
        normalizeField(item, 'caption');
      });
    }
