...
const SAFE_IMAGE_SRC_PATTERN = /^(https?:\/\/|cid:)/i;
...
function stashHtml(stash, html) {
  const token = `@@CORRESPONDENCE_HTML_${stash.length}@@`;
...
function normalizeSignature(value) {
  ...
  const lines = [];
  if (value.name) lines.push(String(value.name).trim());
  ...
  return cleanLines.length ? { lines: cleanLines } : null;
}

function renderSignatureHtml(signature) {
  if (!signature?.lines?.length) return '';

  return signature.lines
    .map((line, index) => {
      const className = index === 0 ? 'correspondence-signature-name' : 'correspondence-signature-line';
      return `<div class="${className}">${renderInlineMarkdown(line)}</div>`;
    })
    .join('\n');
}
...
export function normalizeCorrespondenceEmailData(source, { inputPath, outputName } = {}) {
  ...
  const brand = source.brand && typeof source.brand === 'object' && !Array.isArray(source.brand)
    ? {
        name: String(source.brand.name || '').trim(),
        url: sanitizeHref(source.brand.url) || '',
      }
    : {};
