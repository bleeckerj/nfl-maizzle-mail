...
import { buildCorrespondenceEmail } from '../../scripts/build-correspondence.mjs';
...
function expandSectionStyleEntries(sectionStyles = {}) {
  return Object.entries(sectionStyles).flatMap(([key, def]) =>
    key
      .split(',')
      .map((type) => type.trim())
      .filter(Boolean)
...
function readTemplateCapability(dir, { palette = [] } = {}) {
  const manifestPath = path.join(dir, 'template.manifest.json');
  if (fs.existsSync(manifestPath)) {
    const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
    return {
      manifestPath,
      supportedPublicationModes: manifest.supportedPublicationModes || manifest.supported_modes || [],
      starterMarkdown: manifest.starterMarkdown || manifest.starter_markdown || {},
      supportedSections: manifest.supportedSections || manifest.supported_sections || palette.map((entry) => entry.type),
      registryHydrations: manifest.registryHydrations || manifest.registry_hydrations || [],
      footerBehavior: manifest.footerBehavior || manifest.footer_behavior || {},
    };
  }
  return {
    manifestPath: null,
    supportedPublicationModes: ['public-issue'],
    starterMarkdown: { campaign: null },
    supportedSections: palette.map((entry) => entry.type),
    registryHydrations: [],
    footerBehavior: { 'public-issue': 'template-defined footer', campaign: 'unsupported' },
  };
}
    ...
    handler: async () => {
      ...
      for (const e of entries) {
        ...
        if (fs.existsSync(reportPath)) {
          ...
          palette = expandSectionStyleEntries(s.sectionStyles || {}).map(([type, def]) => ({
            type,
        ...
        templates.push({
          ...
          palette,
          capability: readTemplateCapability(dir, { palette: palette || [] }),
        });
      }
  ...
  return {
    definition: {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          output_dir: {
            type: 'string',
            description: 'Optional output directory for a regular public-issue build.',
          },
        },
        required: ['content_md_path'],
      },
    ...
    handler: async (args) => {
      ...
      if (mdPath.endsWith('.md')) {
        sourceFrontmatter = matter(fs.readFileSync(mdPath, 'utf8')).data || {};
      } else if (mdPath.endsWith('.json')) {
        sourceFrontmatter = JSON.parse(fs.readFileSync(mdPath, 'utf8')) || {};
      }
      if (sourceFrontmatter.publicationMode === 'campaign') {
        throw new Error('Campaign sources must use build_campaign_email; build_newsletter remains the regular issue path.');
      }
      const cliArgs = [
        ...
        ...(args.template ? [`--template=${args.template}`] : []),
        ...(args.output_dir ? [`--output-dir=${resolveInRepo(repoRoot, args.output_dir)}`] : []),
        '--no-open',
      ];
...
export function buildCampaignEmail({ repoRoot }) {
  return {
    definition: {
      name: 'build_campaign_email',
      description:
        'Build an explicit campaign Markdown source through the canonical Maizzle pipeline. Returns HTML, link-tracking manifest, and content-slot manifest paths without selecting a template or public output path implicitly.',
      inputSchema: {
        type: 'object',
        properties: {
          content_md_path: { type: 'string' },
          campaign_name: { type: 'string', description: 'Output filename stem; defaults to the source filename.' },
          template: { type: 'string', description: 'Optional assertion that must match source frontmatter template.' },
          output_dir: { type: 'string', description: 'Output directory; defaults to output/email-campaigns/<campaign_name>.' },
        },
        required: ['content_md_path'],
      },
    },
    handler: async (args) => {
      const mdPath = resolveInRepo(repoRoot, args.content_md_path);
      if (!fs.existsSync(mdPath)) throw new Error(`Markdown file not found: ${mdPath}`);
      const rawSource = fs.readFileSync(mdPath, 'utf8');
      const parsed = matter(rawSource);
      const frontmatter = parsed.data || {};
      if (frontmatter.publicationMode !== 'campaign') {
        throw new Error('Campaign source must declare publicationMode: campaign.');
      }
      const template = typeof frontmatter.template === 'string' ? frontmatter.template.trim() : '';
      if (!template) {
        throw new Error('Campaign source must declare an explicit template.');
      }
      if (args.template && args.template !== template) {
        throw new Error(`Campaign template mismatch: source declares "${template}" but requested "${args.template}".`);
      }
      const outputName = args.campaign_name || path.basename(mdPath, path.extname(mdPath));
      if (!/^[a-z0-9][a-z0-9-]*$/i.test(outputName)) {
        throw new Error('campaign_name must contain only letters, numbers, and hyphens.');
      }
      const sourceRoot = path.basename(path.dirname(mdPath)) === 'email-campaigns'
        ? path.dirname(path.dirname(mdPath))
        : repoRoot;
      const defaultOutputRoot = path.join(sourceRoot, 'output', 'email-campaigns');
      const outputDir = args.output_dir
        ? resolveInRepo(repoRoot, args.output_dir)
        : path.join(defaultOutputRoot, outputName);
      const normalizedOutputDir = path.resolve(outputDir);
      if (normalizedOutputDir.split(path.sep).includes('public')) {
        throw new Error(`Campaign output must stay outside public/: ${normalizedOutputDir}`);
      }
      const sourceHashBefore = createHash('sha256').update(rawSource).digest('hex');
      const cliArgs = [
        'scripts/build-newsletter.mjs',
        mdPath,
        outputName,
      ...
      const { stdout, stderr } = await execFileP('node', cliArgs, {
        ...
        throw new Error(`build_campaign_email failed (exit ${error.code}):\n${output}`);
      });
      const sourceHashAfter = createHash('sha256').update(fs.readFileSync(mdPath)).digest('hex');
      const artifacts = {
        html: path.join(normalizedOutputDir, `${outputName}.html`),
        linkManifest: path.join(normalizedOutputDir, `${outputName}.link-tracking-manifest.json`),
        contentSlotManifest: path.join(normalizedOutputDir, `${outputName}.content-slots.json`),
      };
      const missing = Object.values(artifacts).filter((file) => !fs.existsSync(file));
      if (missing.length) {
        throw new Error(`Campaign build did not produce expected artifacts: ${missing.join(', ')}`);
      }
      return json({
        success: true,
...
export function buildCorrespondence({ repoRoot }) {
  return {
    definition: {
      name: 'build_correspondence',
      description: 'Compose and render a standard correspondence email from its structured payload. Returns rendered HTML and normalized metadata; never sends email.',
      inputSchema: {
        type: 'object',
        properties: {
          subject: { type: 'string' },
          ...
          sharedItems: { type: 'array', items: { type: 'object', additionalProperties: true } },
          sharedItemsHeading: { type: 'string' },
          footerNote: { type: 'string' },
          footerLinks: { type: 'array', items: { type: 'object', additionalProperties: true } },
          theme: { type: 'object', additionalProperties: true },
          showSubject: { type: 'boolean' },
          showFromHeader: { type: 'boolean' },
        },
        required: ['subject'],
      },
    },
    handler: async (args) => {
      const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'nfl-correspondence-'));
      const inputPath = path.join(tempRoot, 'correspondence.json');
      const outputDir = path.join(tempRoot, 'output');
      fs.writeFileSync(inputPath, JSON.stringify({ template: args.template, ...args }), 'utf8');
      ...
      try {
        ...
        const result = await buildCorrespondenceEmail({
          args: [inputPath, 'mcp-correspondence', `--repo-root=${repoRoot}`, `--output-dir=${outputDir}`, '--no-open'],
          openPreview: false,
          logger: { log() {}, warn() {} },
        });
        const html = fs.readFileSync(result.finalOutputPath, 'utf8');
        const text = html.replace(/<style[\s\S]*?<\/style>/gi, '').replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();
        return json({ template: result.templateName, subject: result.data.subject, htmlBody: html, textBody: text });
      } finally {
...
function loadTemplatePalette(repoRoot, name) {
  ...
  if (fs.existsSync(stylesPath)) {
    ...
    const types = new Map();
    for (const [type, def] of expandSectionStyleEntries(styles.sectionStyles || {})) {
      types.set(type, {
        type,
        displayName: def.name || type,
