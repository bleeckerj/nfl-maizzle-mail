...
${darkModeHead}  <title>{{ page.title || title || '${templateName}' }}</title>
  <style>
    /* Mobile font-size lock — sets body copy explicitly on small screens.
       Same pattern as dense-discovery. Don't change these without updating
       tests/newsletter-core/mobile-font-size-lock.test.mjs. */
...
function buildSectionStyles(components, sectionAssignments, segments, { darkModeFlatten = true } = {}) {
  ...
  const globalOverrides = {
    description:
      'Mobile font-size lock — sets body copy explicitly on small screens. Same pattern as dense-discovery.',
