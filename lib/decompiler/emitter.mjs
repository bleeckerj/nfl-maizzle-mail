...
export function emit({ repoRoot, templateName, sourcePath, segmentation, classifier }) {
  ...
  fs.writeFileSync(layoutPath, layoutHtml(templateName));

  const newsletterPath = path.join(templateDir, 'newsletter.html');
  fs.writeFileSync(newsletterPath, newsletterHtml(templateName, components, componentFiles));

  const sectionStyles = buildSectionStyles(components, sectionAssignments, sections);
  fs.writeFileSync(
...
function layoutHtml(templateName) {
  return `<!DOCTYPE html>
...
<head>
  ...
  <title>{{ page.title || title || '${templateName}' }}</title>
  <style>
...
function newsletterHtml(templateName, components, _componentFiles) {
  ...
  const branches = components
    .map((c) => {
      const body = addMobTextClass(rewriteSlots(c.template, c.slots));
...
function buildSectionStyles(components, sectionAssignments, segments) {
